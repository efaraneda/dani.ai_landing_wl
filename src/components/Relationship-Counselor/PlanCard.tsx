...
export default function PlanCard({
  ...
  return (
    ...
    <a className="text-2xl font-light mt-1">{price === '3.5' ? '50' : '00'}</a>
      ...
      {inputVisible && (
        <div className="-mt-4">
          <p className="text-bizantino text-xs font-semibold px-auto mb-0.5 text-white">* Please verify your phone number is correct</p>
          <input
            type="text"
            className="outline-2 text-center outline outline-bizantino rounded-lg border-black border w-[255px]"
            placeholder="Example: (786) 317-8241"
            value={phoneNumber}
...
>
  Buy Now
</button>{isPaymentInProgress && <div className='flex items-center justify-center mt-1 text-white'>Loading<Spinner/></div>} {/* Render Spinner only when isLoading is true */}
        </div>
      )}
    </div>
  );
