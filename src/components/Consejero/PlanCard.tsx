    ...
    <a className="text-2xl font-light mt-1">{price === '3.5' ? '50' : '00'}</a>
      ...
      <ul className="mb-10 flex flex-col gap-y-2">
        {features.map((feature) => (
          <li className="flex items-center text-white" key={feature}>
      ...
      {buttonClicked ? null : (
        <button
          className="mt-auto rounded-xl bg-crayola py-3 px-6 text-lg font-medium text-white hover:bg-bizantino"
      ...
      {inputVisible && (
        <div className="-mt-4">
          <p className="text-bizantino text-xs font-semibold px-auto mb-0.5 text-white">* Por favor, verifica que tu número sea el correcto</p>
          <input
            type="text"
            className="outline-2 text-center outline outline-bizantino rounded-lg border-black border w-[255px]"
            placeholder="Ejemplo: +5693178241"
            value={phoneNumber}
            onChange={handleInputChange}
          />

<button
  className="mt-1 rounded-lg bg-crayola  py-1 w-[255px] text-sm font-medium text-white hover:bg-bizantino"
  onClick={handleValidationAndPayment}
  disabled={isPaymentInProgress} // Deshabilitar el botón mientras el pago está en progreso
>
  Pagar
</button>{isPaymentInProgress && <div className='flex items-center justify-center mt-1 text-white'>Cargando<Spinner/></div>} {/* Render Spinner only when isLoading is true */}
        </div>
      )}
    </div>
  );
